...
- (void)ready:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options {
    ...
    float rawScale = pow(1.3, [defaults floatForKey:@"scaleSlider"] * 2.0 - 1.0);
...
- (void) scaleChanged {
    ...
    CGFloat realWidth = self.webView.frame.size.width;
    ...
    NSString* s = [args JSONRepresentation];
    [self writeJavascript:[NSString stringWithFormat:@"ScaleChangedFireEvent(%@);", s]];
